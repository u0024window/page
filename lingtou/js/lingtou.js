		...
		var navulli = document.querySelector("nav ul li");

		var navW = parseInt(window.getComputedStyle(nav , null)['width']);
		var navli = parseInt(window.getComputedStyle(navulli , null)['width']);
		...
		var eArticle = document.querySelector('.arti');
			...
			if(eArticle.innerHTML.toString().length/26<3){
		...
		function touchendHandler(event){
			...
			if(nowX > 0){
				nowX = 0;
			}else if(nowX < -parseInt(navul.style.width)*24 + navW){
